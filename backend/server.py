...
def execute_command():
    """Receive a command, validate it, and inject into the tmux session."""
    ...
    if len(cmd) > MAX_CMD_LEN:
        return jsonify({
            "ok": False,
            "error": f"Command too long (max {MAX_CMD_LEN} chars)",
        }), 400

    # Rate limit
    now = time.time()
    if now - last_command_time < RATE_LIMIT:
        remaining = round(RATE_LIMIT - (now - last_command_time), 1)
        return jsonify({
            "ok": False,
    ...
    try:
        ...
        return jsonify({
            "ok": False,
            "error": f"tmux execution failed: {exc}",
        }), 500
